...
function AppVal() {
  ...
  //Added to check for binary calculations
  const [checkBinaryMode, setBinaryMode] = useState(false)
      ...
      switch (nextAction) {
        ...
        case "Action_show_result":
          ...
          if(checkBinaryMode){
            ...
            var i = operandOne.length - 1, j = operandTwo.length - 1;
            while (i >= 0 || j >= 0 || s == 1) {

              // Comput sum of last
              // digits and carry
              s += ((i >= 0) ? operandOne.charAt(i).charCodeAt(0) -
                '0'.charCodeAt(0) : 0);
              s += ((j >= 0) ? operandTwo.charAt(j).charCodeAt(0) -
                '0'.charCodeAt(0) : 0);
